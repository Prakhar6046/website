...
export const ImageMedia: React.FC<MediaProps> = (props) => {
  ...
  const sizes = sizeFromProps
    ...
    : Object.entries(breakpoints)
        .map(([, value]) => `(maxWidth: ${value}px) ${value * 2}w`)
        .join(', ')
  ...
  return (
    <>
      {expanded &&
        ReactDOM.createPortal(
          <picture
            ...
            className="fixed inset-0 z-50 h-full w-full cursor-pointer bg-card/40 p-12 backdrop-blur-sm"
          >
            <AnimatePresenceBlock className="relative">
              <NextImage
                alt={alt || ''}
                className={cn('relative z-10 mx-auto h-full object-cover')}
                fill={fill}
      ...
      <picture className={''}>
        <NextImage
          alt={alt || ''}
          className={cn(imgClassName, isExpandable && 'cursor-pointer')}
          fill={fill}
          onClick={(e) => {
